...
const MinorGroupManagement = () => {
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get("/api/v1/minor/get-req");
  ...
  const createGroup = async () => {
    setLoading(true);
    try {
      const response = await axios.post("/api/v1/minor/create-group", {});
      setGroup(response.data.data);
  ...
  const addMember = async () => {
    ...
    try {
      await axios.post("/api/v1/minor/add-member", {
        rollNumber,
        groupId: group?._id,
      });
  ...
  const removeMember = async (rollNumber) => {
    try {
      await axios.post("/api/v1/minor/remove-member", {
        rollNumber,
        groupId: group?._id,
      });
  ...
  const acceptRequest = async (groupId) => {
    try {
      await axios.post("/api/v1/minor/accept-req", { groupId });
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 text-white">
              ...
              <p className="text-blue-100 mt-1">
                {group
                  ...
                  : "Create your minor project group"}
              </p>
            </div>
                      ...
                      <div className="grid md:grid-cols-3 gap-4">
                        ...
                        <div className="bg-purple-50 rounded-lg p-4 border border-purple-100">
                          ...
                          <p className="text-xl font-bold text-purple-600 mt-1">
                            {group?.minorAllocatedProf?.fullName ||
                              "Not Allocated"}
                          </p>
                        </div>
                      </div>
...
export default MinorGroupManagement;
