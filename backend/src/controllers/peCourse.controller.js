...
export const addPeCourse = async (req, res) => {
  try {
    ...
    if (!peCourseIVId || !peCourseVId) {
      return res
        .status(400)
        .json({ success: false, message: "Missing PE course information." });
    }
    ...
    const course1 = await PeCourse.findOne({
      courseCode: peCourseIVId.toString(),
      batch: batchToUse,
    });
    const course2 = await PeCourse.findOne({
      courseCode: peCourseVId.toString(),
      batch: batchToUse,
    });
    ...
    res
      .status(200)
      .json({ success: true, message: "PE course added successfully." });
  } catch (error) {
...
export const getAdminPeCourses = asyncHandler(async (req, res) => {
  try {
    ...
    if (!courses || courses.length === 0) {
      return res
        .status(404)
        .json(new ApiResponse(404, [], "No PE courses found."));
    }
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          courses,
          "All PE course data with students fetched successfully."
        )
      );
  } catch (error) {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          500,
          null,
          "An error occurred while fetching PE courses."
        )
      );
  }
});
